...
// Added function to translate transcribed text after transcription
function translateTranscribedText(transcribedText) {
    let fromContent = langOption[0].value;
    let transContent = langOption[1].value;

    let transLINK = `https://api.mymemory.translated.net/get?q=${transcribedText}&langpair=${fromContent}|${transContent}`;
    fetch(transLINK).then(translate => translate.json()).then(data => {
        transText.value = data.responseData.translatedText;
    });
}
...
recognition.onresult = function(event) {
    ...
    // Translate the transcribed text after it's completed
    translateTranscribedText(transcript);
};
